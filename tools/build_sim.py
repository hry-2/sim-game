...
/* 菜单/兴造开着时，摇杆和「合」「造」都用不上，收起来别压着菜单。
   留下「✕」取消和「互动」确认 —— 那两个正是菜单的操作键。 */
body.touch.menuing #tcpad,
body.touch.menuing #tccraft,
body.touch.menuing #tcbuild{display:none}
  ...
  <div id="stage">
    ...
    <div id="tc">
      ...
      <div class="btns">
        ...
        <button id="tccraft" class="sm">合</button>
...
// 画面尽量吃满屏。视野是【整格】的，所以这个数要给足才跨得过下一格 ——
// 844 高的手机上：0.90 只给到 760，而 17 格要 816，会被量化砍回 15 格（85%）；
// 给到 0.99 才够 17 格（97%）。事件流和面板滚下去看，底下留的那点缝正好当个提示。
const TFILL=0.99;
function fit(){
  ...
  best = best || {sc:1, w:Math.min(GW,minW), h:Math.min(GH,minH)};
...
// 一人一天吃三顿上下、多数菜费一份米 —— 十二份约合四天的口粮，够了就别再囤
const RICE_ENOUGH=12;
...
const MKT={
  ...
  meat     :{p:26, base:26, lo:0.55, hi:1.6},   // 兽肉：来得险，也坏得最快
  hide     :{p:34, base:34, lo:0.6,  hi:1.5},   // 兽皮：不坏，是打野物真正的进项
};

// ==========================================================================
//  钱得有下水道
//  在此之前钱只进不出：木器、兽皮、腌货卖了钱，NPC 除了籴米（还封了顶）
//  和聘礼/馈赠之外没处花 —— 实测 28 天人均从 125 涨到 174，村子在变成
//  一屋子守财奴。而「守财奴」不是玩法，只是一个没人管的数字。
//
//  下水道开在【公共去处】上：市井里吃茶要茶钱、洗澡要澡钱，本来就该收。
//  两条讲究：
//    · 只收【可选】的那些。茅厕免费 —— 内急掉得最快，收钱等于罚款；
//      水井也免费 —— 洗得差一半多（38 对 88），但穷人总得有地方洗把脸。
//    · pre 挡住没钱的人，于是穷人自动退到水井。贫富差第一次能被【看见】：
//      有钱的人干净体面，没钱的人凑合 —— 这比一个只涨不落的数字有意思。
//  不发事件：一天几百次吃茶洗澡，全写进事件流就把故事淹了。玩家那边给浮字。
// ==========================================================================
const PRICE={ bath:5, tea:6 };
// 有闲钱才吃茶：兜里得先留出一份米钱，不然就是把饭钱喝了。
// 【不留这一手的话】开局六十文，头两天全花在澡堂茶棚上，第三天买不起米 ——
// 冒烟测试里 starving 当场冒了一个。这不是把价钱调低能解决的，
// 缺的是"先顾肚子"这条家用常识：效用 AI 只比较此刻的分数，不会自己留钱。
function canAfford(s, n){
  if(s.money < n) return false;
  // 家当还没置办齐的人，闲钱得先留着安家 —— 没个窝就别先进澡堂。
  // 【不留这一手的话】托管的玩家会把钱花在茶棚澡堂上，攒不出立米缸的十文，
  // 于是守着十三斗米和三根柴饿肚子（有粮、有柴，没锅没缸）。
  const owe = homeGap(s);
  if(owe > 0) return s.money - n >= owe;
  if(household(s).food >= 3) return true;      // 家里有米，闲钱随便花
  return s.money - n >= MKT.food.p;            // 没米：花完还得买得起一份米
}
// 还差多少钱才能把缺的那件家当立起来（0 = 齐了 / NPC 本来就自带）
function homeGap(s){
  if(s !== PC) return 0;
  for(const [key, art] of AUTO_HOME){
    const has = OBJ.some(o=>o.built && o.owner===s.name &&
                            (art==='stove' ? !!o.stove : artOf(o)===art));
    if(!has) return BUILD[key].money;
  }
  return 0;
}
function spend(s, n){
  s.money-=n;
  if(s===PC) fx(s.rx||s.px, (s.ry||s.py)-CH, '-'+n+' 文', '#d0b070');
  return null;                       // 静默：settleEff 收到 null 就不写事件流
}
...
function mktSellHide(s){                          // 售一张皮
  const v=Math.round(MKT.hide.p*(boonOn('市面')?1.12:1));
  s.money+=v; s.inv.hide--;
  MKT.hide.p=Math.max(MKT.hide.base*MKT.hide.lo, MKT.hide.p*0.95);
  return v;
}
function mktSellMeat(s){                          // 售一块肉
  const v=Math.round(MKT.meat.p*(boonOn('市面')?1.12:1));
  s.money+=v; s.inv.meat--;
  MKT.meat.p=Math.max(MKT.meat.base*MKT.meat.lo, MKT.meat.p*0.95);
  return v;
}
...
// ==========================================================================
//  第 0 块地是【玩家的空地】：不盖房、不围篱笆、地上什么都没有。
//  开局你只有一身衣裳和几根柴 —— 榻、灶台、米缸、田、树，全要自己造。
//  其余五户照旧有屋有院：这是个市井，不是荒岛；对照才看得出你在从头起家。
// ==========================================================================
const PLOT0=0;
HOME.forEach((h,hi)=>{
  if(hi===PLOT0) return;
...
// 行囊里东西的名字。inv 的键是英文，界面上一直缺一张对照表 ——
// 以前只有「家赀排名」那行小字里硬写了「米/木/器」，别的东西玩家根本看不见。
const INV_N={ food:'米', wood:'柴', goods:'木器', fish:'鱼', egg:'蛋',
              pickle:'腌货', stone:'石料', meat:'兽肉', hide:'兽皮',
              veg:'野菜', fruit:'野果', mush:'菌子', herb:'药草' };
// 存档是按 inv 整份读回来的，老档里没有后加的键。不补齐的话
// s.inv.meat++ 会变成 NaN，一路污染到卖价和面板 —— 加键就要在这儿兜一次。
const INV_KEYS=Object.keys(INV_N);
function fixInv(inv){ const o={}; for(const k of INV_KEYS) o[k]=(inv&&inv[k])|0; return o; }
...
function mkTree(id,x,y){
  return { id, zone:'work', n:'林地', art:'tree', tree:1, gives:{wood:5},
    x, y, w:1, h:1, use:[[x,y+1]],
    col:['#3d4a2c','#2b351f','#4a3628'],
    adv:{provision:26}, dur:90, cost:{energy:16,hygiene:12},
    // 一趟 2 根只够烧两顿 —— 那是天天得跑的苦役，效用 AI 会（很合理地）
    // 选择在家吃冷食。一趟 5 根约合一个人一天半的柴，砍柴才成为「日常活计」
    // 而不是「填不满的窟窿」。
    skill:true,     // 力气活也长手艺：抡了一天斧头还是「生手」，说不过去
    eff:s=>{ s.inv.wood += hasTool('斧',s)?7:5; },
    get note(){ return '伐木 +'+(TOOLS['斧']?7:5); } };
}
// 采石场：T36 的新资源。石料不开新的加工链，只做【更大更贵的建造】的料。
function mkQuarry(id,x,y,seat){
  const sp={cap:3, left:3}; DAILY.push(sp);
  return { id, zone:'work', n:'采石场', art:'quarry', quarry:1, daily:sp, gives:{stone:1},
    skill:true,     // 同上
...
const RECIPE=[
  ...
  // ---- 后加的六道 ----
  // 挑选的标准不是"多几行"，而是每一道都要补上一个【原来没有的处境】：
  //   稀粥   —— 只剩一斗米时，家常饭的两斗做不成，以前只能吃冷食
  //   野菜团 —— 春天的山货第一次能直接下锅（不用配米）
  //   果羹   —— 闲趣最高的一道，夏天的野果有了第二个去处
  //   菌鱼羹 —— 秋天：不费米的好菜
  //   腌菜饭 —— 腌货【第一次能吃】。以前它只是个 40 文的卖品，
  //             现在"吃掉还是卖掉"成了一个真的选择
  //   八宝饭 —— 跨季攒料才做得成的大菜（野果夏、菌子秋、蛋要鸡棚）
  { n:'稀粥',   need:{food:1},            adv:{hunger:44},            note:'米×1（省米）' },
  { n:'野菜团', need:{veg:2},             adv:{hunger:58},            note:'野菜×2（不费米）' },
  { n:'果羹',   need:{fruit:2},           adv:{hunger:46, fun:34},    note:'野果×2（不费米）' },
  { n:'菌鱼羹', need:{fish:1, mush:1},    adv:{hunger:82, energy:20}, note:'鱼×1 菌子×1（不费米）' },
  { n:'腌菜饭', need:{food:1, pickle:1},  adv:{hunger:88, fun:8},     note:'米×1 腌货×1' },
  { n:'八宝饭', need:{food:2, fruit:1, mush:1, egg:1},
                adv:{hunger:100, fun:20, energy:15},                  note:'米2 野果1 菌子1 蛋1' },
  // 打野物换来的两道。肉坏得最快（0.5/天），所以它是「当天就得处理掉」的东西
  { n:'烤肉',   need:{meat:1},            adv:{hunger:76, energy:16}, note:'兽肉×1（不费米）' },
  { n:'肉羹',   need:{food:1, meat:1},    adv:{hunger:94, energy:24}, note:'米×1 兽肉×1' },
];
// 手艺也是一味"料"：手生的人做不出腌菜饭和八宝饭。
// NPC 的 bestRecipe() 走的就是这里，所以这一条对全村生效，不是只管玩家。
const canMake=(s,r)=>(!r.lv || skillLv(s)>=r.lv)
  && Object.keys(r.need).every(k=>(s.inv[k]||0)>=r.need[k]);
...
// 这次行动实际广播多少需求。
// 【菜的饱腹要以菜谱为准，不是灶台那个固定的 78】——
// 不改的话 RECIPE 里那一列 hunger 只是喂给 bestRecipe() 排序的摆设，
// 烤鱼(66) 和八宝饭(100) 吃下去一模一样，加再多菜谱也吃不出区别。
function advOf(s){
  const a=s.act, ad=(a&&a.o&&a.o.adv)||{};
  return (a && a.recipe && a.recipe.adv && ad.hunger!=null)
    ? Object.assign({}, ad, {hunger:a.recipe.adv.hunger}) : ad;
}
function cook(s, forced){
  const r = forced || bestRecipe(s);
  if(!r) return null;
  s.inv.wood=Math.max(0,(s.inv.wood|0)-1);        // 烧掉一根柴
...
const OBJ=[
 ...
 {id:'bath1', zone:'home', n:'浴堂',x:9, y:27,w:1,h:1, use:[[9,28]], col:['#7fa6ad','#547278','#b6d2d6'],
  ...
  pre:s=>canAfford(s,PRICE.bath),
  why:s=>s.money<PRICE.bath?'澡钱不够（水井打水不要钱）':'米钱还没着落，先顾肚子',
  eff:s=>spend(s,PRICE.bath), get note(){ return '沐浴 '+PRICE.bath+' 文'; }},
 ...
 {id:'table', zone:'commons', n:'茶棚',x:29,y:28,w:2,h:2, use:[[28,29],[31,29]], col:['#7a5a3c','#54402c','#8d6b48'],
  adv:{social:42,fun:24,hunger:14}, dur:60, social:true,
  // 【茶棚不设闸】：这游戏的内核是人跟人的关系，把它卡在钱后面代价太大 ——
  // 实测收钱挡掉了 22% 的社交，而且【外向的人被挡得更多】（15% 对 6%），
  // 正好把 MBTI 那条"E 比 I 社交多"的涌现信号压平了。
  // 改成「有钱添碗茶，没钱白坐」：位子不要钱，钱照样从手头宽裕的人身上流走 ——
  // 而且流的正是富人（穷人本来也付不起），这恰恰是原来最缺下水道的那一头。
  eff:s=>{ if(s.money>=PRICE.tea) spend(s,PRICE.tea); return null; },
  get note(){ return '吃茶闲话（有钱添碗茶 '+PRICE.tea+' 文）'; }},
 ...
 {id:'bench', zone:'work', n:'木作坊',x:4, y:31,w:2,h:1, use:[[4,32],[5,32]], col:['#8a6a42','#5e482c','#b89a5e'],
  adv:{provision:44}, dur:120, mat:{wood:2}, matN:{wood:'木料'}, eff:s=>craft(s), craft:1,
 ...
 {id:'stall', zone:'work', n:'集市',x:15,y:31,w:2,h:1, use:[[15,32],[16,32]], col:['#9a4a3a','#6e3328','#d8b45e'],
  ...
  pre:s=>s.inv.goods>=1 || s.inv.pickle>=1 || s.inv.egg>=1 || s.inv.fish>=1
        || s.inv.hide>=1 || s.inv.meat>=1,
  why:'手里没什么可卖的',
  // 按单价排：木器 > 兽皮 > 腌货 > 兽肉 > 蛋 > 鱼。
  // 兽肉排在腌货后面不是因为便宜，是因为它坏得最快 —— 先出手不坏的没道理
  eff:s=> s.inv.goods>=1  ? {type:'econ.sold',       val:mktSell(s)}
        : s.inv.hide>=1   ? {type:'econ.soldhide',   val:mktSellHide(s)}
        : s.inv.pickle>=1 ? {type:'econ.soldpickle', val:mktSellPickle(s)}
        : s.inv.meat>=1   ? {type:'econ.soldmeat',   val:mktSellMeat(s)}
        : s.inv.egg>=1    ? {type:'econ.soldegg',    val:mktSellEgg(s)}
                          : {type:'econ.soldfish',   val:mktSellFish(s)},
  note:'售木器/兽皮/腌货/兽肉/蛋/鲜鱼（随行就市）'},
 {id:'shop', zone:'work', n:'米铺',x:20,y:31,w:1,h:1, use:[[20,32]], col:['#7a6a4a','#54492f','#c9a86a'],
  // 籴米广播的是常数 60，而 provision() 里存粮九份就封顶 —— AI 感觉不到自己
  // 在囤。实测 14 天籴 423 份、吃掉 390 份、田里又收 75 份，全村存粮 209→317
  // 一路只涨，钱全变成了米山。所以要一条"够吃四天就别再籴"的刹车 ——
  // 但它挂在 want 上而不是 pre 上：那是 AI 的自觉，不是柜台不卖。
  adv:{provision:60}, dur:30, eff:s=>({type:'econ.bought', val:mktBuy(s)}),
  pre:s=>s.money>=MKT.food.p,
  want:s=>household(s).food >= RICE_ENOUGH*(spouseOf(s.name)?2:1) ? 0.04 : 1,
  why:s=>'差 '+Math.ceil(MKT.food.p-s.money)+' 文',
  get note(){ return '籴米（随行就市，一次三斗）'; }},
...
NAMES.forEach((nm,i)=>{
  const h=HOME[i];
  if(i===PLOT0) return;                 // 玩家那块地上什么都没有：榻/米缸/灶台/田都得自己造
  ...
  OBJ.push({ id:'jar'+(i+1), zone:'home', homeIdx:i, get owner(){ return HOMEOWNER[i]; }, n:'米缸',
    ...
    // 冷食是【唯一不用生火】的一口吃的，所以它不能只认米。
    // 只认米的话，手里攥着一把野果、灶下没柴的人就得先跑一趟北山才吃得上东西 ——
    // 没柴可以吃得差（38 对灶台的 78），不该吃不上。烤鱼那条"断粮的后路"
    // 现在要柴，这里就得替它把底兜住。
    pre:s=>s.inv.food>=1||s.inv.fruit>=1, why:'缸里没米，也没果子',
    eff:s=>{ if(s.inv.food>=1) s.inv.food--; else s.inv.fruit--; },
    note:'取些冷食（不用柴）' });
  OBJ.push({ id:'stove'+(i+1), zone:'home', homeIdx:i, get owner(){ return HOMEOWNER[i]; }, n:'灶台',
    ...
    mat:{wood:1},                       // 烧一顿要一根柴 —— 木料由此挂上「饿」
    plan:s=>bestRecipe(s),              // 开工前先定下做哪道，行动中才按它回复
    pre:s=>!!bestRecipe(s), eff:s=>cook(s, s.act&&s.act.recipe), note:'生火做饭（费柴×1）',
...
NAMES.forEach((nm,i)=>{                       // 自家院里那块，安稳
  const h=HOME[i];
  if(i===PLOT0) return;                       // 玩家那块地连田都没有开 —— 「新田」得自己造
...
function household(s){
  ...
  return p ? {food:(s.inv.food|0)+(p.inv.food|0), money:s.money+p.money,
              wood:(s.inv.wood|0)+(p.inv.wood|0)}
           : {food:s.inv.food|0, money:s.money, wood:s.inv.wood|0};
}
// 柴米油盐，柴排在头一个 —— 家里没柴，日子就不算过得下去。
// 把它算进家计，伐木才有一个【会自己变紧张】的雇主：
// 光靠「缺料传递」不够，那份传递的分数跟着【此刻】的饿劲走，
// 而饿了才想起砍柴已经太晚 —— 人会（很合理地）就近抓一把冷食。
// 家计里的这一项才是「未雨绸缪」：柴堆见底，不饿也心慌。
function provision(s){ const H=household(s); return Math.max(0, Math.min(100,
  Math.min(s===PC&&BARN.n>0?76:48, H.food*6)         // 谷仓：米囤得住了
  + Math.min(16,(H.wood|0)*4)                        // 柴堆
...
// ==========================================================================
const TOOL={
  斧:{ n:'铁斧', wood:4, money:60, dur:150, tip:'伐木 5→7 根，工时省两成' },
  ...
  叉:{ n:'猎叉', wood:3, money:50, dur:130, tip:'打野物一下抵两下' },
  甲:{ n:'皮甲', wood:2, hide:3, money:40, dur:150, tip:'兽皮×3。野物咬一口，伤减一半' },
};
let TOOLS={};                                   // 玩家已有的工具
...
// ==========================================================================
//  节奏调参 —— 「人物干活太慢」的两个真正来源
//  ① 工时：伐木 90、采石 120、木作 120，而工作锚点只有 9~18 点这 540 分钟，
//     扣掉来回走路，一个人一天只干得完三四件事，看着就是在磨蹭。
//  ② 脚程：地图从小院扩到 62×52 之后，横穿一趟就是几十分钟，全花在路上。
//  WORK_MUL 只压时长，不动产出与需求回复的【总量】—— 同一个工时段能多干几件事，
//  单件活的收益不变。想再快往下调（0.4≈原速两倍半），想回到原样设 1。
// ==========================================================================
const WORK_MUL=0.55;                  // 工时倍率：所有活计的时长 × 此值
const WALK=2.0;                       // 脚程：格/模拟分钟（玩家与 NPC 共用）
const CO_MEM=Math.max(12,Math.round(45*WORK_MUL));  // 「一起待了多久」才记一笔，随工时缩

// ==========================================================================
//  力气活：抡起来，别站着看进度条
//  伐木、采石、采集、田里的活、木作坊 —— 这几样是「出力」的活。站着等一根进度条
//  走完，既没手感也没参与感。玩家改成一下一下地抡：按一次 E 落一下，落满就成，
//  想多快就多快。NPC 照旧按工时慢慢磨（他们不会按键）。
//  连带的好处：工具和手艺折的不再是「进度条走得快一点」，而是【少抡两下】——
//  一把好斧头的价值第一次是看得见的。
//  真正换掉的只有「等」：世界的钟不再为这件活空转，但【人该累的照样累】——
//  每一下都按它折合的工时补一份需求衰减（见 swing()），所以砍一棵树掏空你的
//  程度和从前一模一样，省下的纯粹是你坐在椅子上盯进度条的那几十秒。
// ==========================================================================
const SWING_MIN=12;                   // 一下大约抵多少分钟工时
const SWING_CD=130;                   // 毫秒：两下之间的最小间隔，也是长按连抡的节拍
const LABOR={
  tree  :{verb:'砍', hit:'咚', col:'#c9a86a'},
  quarry:{verb:'凿', hit:'铛', col:'#b9b1a3'},
  forage:{verb:'采', hit:'簌', col:'#8fd08a'},
  field :{verb:'锄', hit:'唰', col:'#c8a05a'},
  craft :{verb:'刨', hit:'嗒', col:'#d0a878'},
};
function laborOf(o){
  if(!o) return null;
  if(o.tree)   return 'tree';
  if(o.quarry) return 'quarry';
  if(o.forage) return 'forage';
  if(o.field)  return 'field';
  if(o.craft)  return 'craft';
  return null;
}
// 几下能完事：由这次的实际工时推出来。2~8 下 —— 再多点着累，再少没手感。
function swingsOf(vdur){ return Math.max(2, Math.min(8, Math.round(vdur/SWING_MIN))); }
// 只有【玩家亲手做的整活】才抡：小憩那类变体、以及托管中的玩家都不算
function attachSwing(a){
  const k=laborOf(a.o);
  if(k && (a.scale==null||a.scale===1))
    a.sw={k, need:swingsOf(a.variantDur||a.o.dur), done:0, t:0};
  return a;
}
...
function playerDur(o, base){
  let d = (base==null ? o.dur : base) * WORK_MUL;
...
function makeTool(s, k){
  ...
  if(t.hide && (s.inv.hide|0)<t.hide) return null;      // 皮甲要兽皮
  s.inv.wood-=t.wood; s.money-=t.money;
  if(t.hide) s.inv.hide-=t.hide;
  TOOLS[k]=1;
...
function mkSim(nm, i, o){
  ...
  return {
    ...
    isPlayer: !!o.isPlayer,
    // 开局家底：柴 5 —— 刚够立起第一件家当（米缸 木4·10文）。
    // 给 3 的话，托管的玩家守着十几斗米和三根柴饿肚子（差一根柴立不了缸）；
    // 给 6 的话，教程第 4 步「弄够六根柴」开局就自动打勾，砍树那一课就白教了。
...
function evText(e){
  ...
  switch(e.type){
    ...
    case 'econ.spoiled':    return `${w}的${e.why}坏了，一共扔了 ${e.val} 份`;
    case 'econ.soldhide':   return `${w}卖了一张兽皮，得 ${e.val} 文`;
    case 'econ.soldmeat':   return `${w}卖了一块兽肉，得 ${e.val} 文`;
    case 'beast.came':      return `${e.why}下山了 —— 北山这几天不太平`;
    case 'beast.bit':       return `${w}在北山让${e.why}咬了一口`;
    case 'beast.slain':     return `${w}打死了一只${e.why}`;
    case 'quest.done':      return `【${e.why}】—— 做到了`;
    case 'act.done': {
    ...
    case 'build.done':      return `${w}起了一座${e.why}`;   // 不再限自家院，别写死地点
...
const EXTRA={
  // 按 art 索引，一条顶原来的 bed1~bed6 六条 —— 自己造的榻也吃得到「小憩」
  bed:[{n:'小憩',dur:70,scale:0.35}],
...
// 落一下就崩几点碎屑。反馈要打在物件上，不是打在数字上 —— 这是「打到了」的实感。
// 落点取【人和物的中点】：崩在物件正中会糊住名字和头顶那排格子，崩在接触处才对。
const chips=[];
function chip(x,y,spread,col){
  for(let i=0;i<6;i++) chips.push({
    x:x+(Math.random()-.5)*spread, y:y+(Math.random()-.5)*spread*0.6,
    vx:(Math.random()-.5)*9, vy:-(Math.random()*7+3), t:0, col});
}
...
function menuList(o){
  // EXTRA 原先只认 id，于是自建的书斋（id 是 b7 之类）拿不到「钻研」那一项 ——
  // 改成 id 优先、art 兜底，镇上的书塾和院里的书斋就共用一份
  let list=[{n:o.n, dur:o.dur, scale:1.0}].concat(EXTRA[o.id]||EXTRA[artOf(o)]||[]);
  ...
  // 灶台 / 木作坊的清单从 CRAFT 生成 —— 和合成面板同一份数据，不会对不上
  if(o.stove) return craftMenuRows(o,'stove');    // 认标记不认 id：自己造的灶台 id 是 bd7
  if(o.field && fieldStage(o.field)==='耕'){
    const se=seasonOf(day);
    return CROPS.map(c=>({ n:'种'+c, dur:o.dur, scale:1.0, crop:c,
      rate:CROP[c].grow[se], yield:CROP[c].yield }));
  }
  if(o.craft) return craftMenuRows(o,'bench');   // 木作坊 / 工棚：木器 + 还没打的家什
  return list;
}
// 一个工作台上能做的东西 —— 菜单行和合成面板共用 CRAFT 与 craftLack
function craftMenuRows(o, st){
  return CRAFT.filter(c=>c.st===st && !craftGone(c)).map(c=>{
    const miss=craftLack(PC,c);
    return { n:c.n, dur: c.tool ? TOOL[c.tool].dur : o.dur, scale:1.0,
             recipe:c.recipe||null, tool:c.tool||null, ok:miss.length===0,
             sub: miss.length ? missText(miss) : craftNeedText(c)+'　'+c.tip };
  });
}
...
function runMenu(){
  ...
  if(!it.ok){
    // 合成面板里点了做不了的：把 sub 里那句理由甩到脸上，别默默无反应
    if(menu.kind==='craft') fx(PC.rx||PC.px,(PC.ry||PC.py)-CH, it.sub, '#d8a06a');
    return;
  }
  if(menu.kind==='craft'){
    const c=it.a; closeMenu();
    if(c.build){ startBuild(c.build); return; }
    const n=stationNear(c.st);
    if(n) startCraftAt(n, c);
    return;
  }
  ...
  if(menu.kind==='npc'){
    ...
    PC.act=attachSwing({o, seat:menu.seat.join(), st:menu.seat, phase:'do',
    ...
    if(PC.act && PC.act.sw) swing();   // 起手就是第一下
  }
  closeMenu();
}
...
addEventListener('keydown',e=>{
  ...
  // 长按 E 连抡是特意留的口子；但长按不该把终局屏/结算屏/菜单一路点穿
  if(e.repeat && (ending||daily||menu||bmode)) return;
  if(ending){ resetAndReload(); return; }   // 终局：重开一局
  ...
  if(menu){                                          // 菜单打开时接管全部按键
    ...
    if(e.code==='KeyC'){ if(menu.kind==='craft') closeMenu(); else openCraftMenu(); }
  ...
  if(e.code==='KeyH'){ questHide=!questHide; panel(); }   // 收起/展开引导
...
(function(){
  ...
  const actBtn=document.getElementById('tcact');
  actBtn.addEventListener('click',()=>{
    if(daily) closeDaily(); else if(menu) runMenu();
    else if(bmode) confirmBuild(); else tryInteract();
  });
  // 按住不放就连着抡 —— 手机上一下一下戳六次太累
  let holdIv=null;
  const holdStop=()=>{ if(holdIv){ clearInterval(holdIv); holdIv=null; } };
  actBtn.addEventListener('pointerdown',()=>{ holdStop();
    holdIv=setInterval(()=>{ if(PC.act&&PC.act.sw&&!menu&&!bmode) swing(); else holdStop(); },
                       SWING_CD+20); });
  for(const ev of ['pointerup','pointercancel','pointerleave'])
    actBtn.addEventListener(ev, holdStop);
  ...
  document.getElementById('tccraft').addEventListener('click',()=>{
    if(menu && menu.kind==='craft') closeMenu();
    else if(!bmode) openCraftMenu();
  });
})();
...
function blockReason(o, u, s){
  ...
  { const m=o.mat && matLack(o,s);   // 同一份木料，灶台叫「柴」、木作坊叫「木料」
    if(m) return '要'+((o.matN&&o.matN[m])||MAT_N[m]||m)+'×'+o.mat[m]
                 +'（现有 '+(s.inv[m]|0)+'）'; }
...
// 一件事做完了：产出、事件、清场。抡完最后一下和进度条走完，走的是同一条路。
function finishAct(s){
  const a=s.act, o=a.o;
  const full=(a.scale==null||a.scale===1);
  if(full && o.pre && !o.pre(s)){
    emit('econ.failed',{who:s.name, obj:o.n, objId:o.id, tags:['econ','conflict']});
  } else {
    // 玩家在菜单里选了什么就做什么：指定的菜谱、指定的那份供奉
    let out=null;
    if(a.heal && s.inv.herb>=1){ s.inv.herb--; s.ill=0;
                   out={type:'life.heal', why:'一剂药草'}; }
    else if(a.kid){ const kd=kidOf(s.name);
                   if(kd){ kd.fed=day; out={type:'family.care'}; } }
    else if(a.tool)   out=makeTool(s, a.tool);
    else if(a.shrine) out=offerGive(s, a.shrine) ? null : null;
    else if(a.recipe) out=cook(s, canMake(s,a.recipe) ? a.recipe : null);  // 料没了就改一道
...
// 落一下。够数了当场结算 —— 不用再等进度条走完。
function swing(){
  const a=PC.act, sw=a&&a.sw;
  if(!sw) return false;
  const now=performance.now();
  if(now-sw.t<SWING_CD) return true;          // 连按太快：吃掉这次，但别当成「没在干活」
  sw.t=now;
  const o=a.o, L=LABOR[sw.k], n=sw.need, vdur=a.variantDur||o.dur;
  // 需求、体力、手艺按「下」平摊 —— 一件活干完的总账和走进度条时一模一样
  for(const k in (o.adv||{})) if(k!=='provision')
    PC.need[k]=Math.min(100, PC.need[k]+o.adv[k]/n);
  for(const k in (o.cost||{}))
    PC.need[k]=Math.max(0, PC.need[k]-o.cost[k]/n);
  if(o.skill) PC.skill += (vdur/n)*0.004*(a.skillMul||1)*(boonOn('匠心')?1.35:1);
  // 这一下折合多少工时，就照那些工时补一份需求衰减。
  // 【不补的话力气活就成了白捡】：世界的钟没为它走，人却一点不饿不累，
  // 「睡一觉→秒砍六棵→再睡」能刷出无限木料。补上之后，砍一棵树掏空你的程度
  // 和走进度条时分毫不差 —— 省掉的只是干等，不是代价。
  const wear=boonOn('安泰')?0.92:1, mins=vdur/n;
  for(const k in NEEDS) if(NEEDS[k].dec){
    const per=(k==='social'?mb(PC,'socDec'):1)*(PC.ill?1.3:1);
    PC.need[k]=Math.max(0, PC.need[k]-NEEDS[k].dec*mins*wear*per);
  }
  sw.done++;
  a.left=vdur*(1-sw.done/n);                  // 头顶那根条还留着，只是走成一格一格的
  PC.swingT=1;                                // 挥击前倾，由主循环按真实时间衰减
  PC.face=((o.x+o.w/2)*T>=PC.px)?1:-1;
  chip((PC.px+(o.x+o.w/2)*T)/2, (PC.py+(o.y+o.h/2)*T)/2, T*0.9, L.col);
  fx(PC.rx||PC.px, (PC.ry||PC.py)-CH-6, sw.done>=n?L.verb+'完了':L.hit, L.col);
  if(sw.done>=n) finishAct(PC);
  return true;
}
function tryInteract(){
  if(menu) return;
  if(strike()) return;                              // 跟前有野物：先打它
  if(PC.act){ swing(); return; }                    // 已经在抡了 → 再来一下
  const np=nearbyNpc();
  ...
  PC.ppx=PC.px; PC.ppy=PC.py;                        // 瞬移：清掉插值起点
  const vdur=playerDur(n.o);
  PC.act=attachSwing({o:n.o, seat:n.u.join(), st:n.u, phase:'do',
          left:vdur, variantDur:vdur, scale:1});
  emit('act.start',{who:PC.name, obj:n.o.n, tags:['act']});
  if(PC.act.sw) swing();          // 起手就是第一下：按下去必须有东西发生
}
...
// ==========================================================================
//  原料与「缺料传递」
//  病灶：效用 AI 只会问「这件事【现在】能不能做」，不会问「做了它，下一件事
//  才做得成」。于是生产链的第一环永远没人干 —— 实测 7 天 1577 次决策里，
//  林地当选第一名 2 次（0.13%），73% 的时候排在 12 名开外，全村木料恒为 0；
//  木作坊因此 pre 恒 false，连候选都进不去，木器这条链整条是死的。
//
//  根因有两层，两层都得治：
//  ① 木料只对着【家计】广播，而家计有地板 —— provision() 里存粮那一项
//     九份米就封顶 60，中位数常年 76，urgency 只剩 0.21。而钓点广播
//     provision + fun，fun 是私人需求、永远在掉、不会被别人填满 ——
//     所以钓鱼永远有正分，伐木永远没有。
//     → 治法：给木料接一条【不会饱和】的下游 —— 灶台烧一顿要一根柴。
//       从此木料挂在「饿」上，而不是挂在「家计」上。
//  ② 就算下游值钱，上游也领不到 —— 缺料的活直接被 pre 挡在候选之外。
//     → 治法：这一段。缺料的活先按「假装料够了」算一遍分，把这个分数打个
//       折记到能供这份料的活头上。走一步弯路的代价就是那个折扣。
//
//  没柴不等于饿死：米缸的冷食还在（hunger 38 对灶台的 78）——
//  柴决定的是【吃得好不好】，不是活不活得下去。这条后路必须留着，
//  否则第一天没人有柴，全村原地饿毙。
// ==========================================================================
const MAT_N={wood:'柴', stone:'石料'};
const CHAIN_K=0.9;                 // 走一步弯路的折扣
let _matSeq=0;                     // 每个模拟步 +1：让下面那张表一步只算一次
// 这件事还差哪份料（只报第一份，够用了）
function matLack(o, s){
  if(!o.mat) return null;
  for(const m in o.mat) if((s.inv[m]|0) < o.mat[m]) return m;
  return null;
}
// 此刻每种原料值多少分 = 「就差这份料」的那些活里，假装料够了能打到的最高分
function matValue(sim){
  if(sim._matAt===_matSeq) return sim._mat;
  const v={};
  for(const o of OBJ){
    const m=matLack(o, sim); if(!m) continue;      // 料够了就不用上游帮忙
    const s=score(sim, o, false, true);            // ← true：假装料够了
    if(s!=null && s>0 && s>(v[m]||0)) v[m]=s;
  }
  sim._mat=v; sim._matAt=_matSeq;
  return v;
}
// 供料的活，把下游「就差这份料」的分数打折领回来
function chainBonus(sim, o){
  const v=matValue(sim); let b=0;
  for(const m in o.gives) if(v[m]) b=Math.max(b, v[m]*CHAIN_K);
  return b;
}

function score(sim, o, noAnchor, asIfSupplied){
  ...
  // 缺料闸门：和 pre 分开写，因为「缺的只是料」是【上游能解决】的，
  // 而 pre 里那些（没菜谱、今日已采尽、钱不够）不是。分不开就没法传递。
  if(o.mat && !asIfSupplied && matLack(o,sim)) return null;
  ...
  // 有野物在的时候，山上的活整体降权 —— 这是效用 AI 里「怕」唯一的表达。
  // 【要压的是整片山，不是野物脚下那几格】：只压附近的话，人照样往山的
  // 另一头去，走着走着就撞上了。实测冬天一周仍被咬 47 次、有人体质掉到 39。
  // 离得近的再狠压一档；远处只压到 12%，所以家计真的见了底的人还是会硬着头皮上山 ——
  // 那一趟才是有分量的故事，而不是背景噪音。
  if(BEASTS.length && o.use[0][1] < HILL_Y){
    const u=o.use[0];
    let near=false;
    for(const b of BEASTS)
      if(Math.abs(b.gx-u[0])+Math.abs(b.gy-u[1]) < 10){ near=true; break; }
    s = scale(s, near ? 0.04 : 0.12);
  }
  // 缺料传递：供料的活领一份下游的分数。放在距离/锚点折扣【之前】——
  // 为了下一顿饭跑北山，那趟路该打的折一分不能少。
  if(o.gives && !asIfSupplied) s += chainBonus(sim, o);
  // want：「够了就别再来了」这类自觉走这里，【不要写进 pre】。
  // pre 同时管两件事 —— NPC 要不要去，和玩家能不能用。防囤积只该管前一件；
  // 写进 pre 就等于连玩家一起锁上，而玩家囤米有正当理由（播种一斗一颗、
  // 四时供奉要三斗、义仓捐粮要两斗）。游戏不该替他决定"家里米还够"。
  if(o.want) s = scale(s, o.want(sim));
...
function decide(sim){
  ...
  for(const c of list){
    ...
    if(p){ sim.path=p;
      ...
      const dur=Math.max(4, Math.round((c.o.skill ? c.o.dur*skillSpeed(sim) : c.o.dur)
                                       *WORK_MUL*(sim.ill?1.25:1)));
      sim.act={o:c.o, seat:seat.join(), st:seat, phase:'go', left:dur, variantDur:dur,
               recipe: c.o.plan ? c.o.plan(sim) : null}; return; }   // 出门前就想好做哪道
  }
  sim.act=null;
}
...
// ==========================================================================
//  野物：北山下来的东西
//  饥荒把「出门」变成一个有代价的决定。这里照搬那个意思，但【收在北山】——
//  镇上和自家院里永远安全，危险只在 y < HILL_Y 那一带。
//  这样做不是偷懒：给全图铺一层压力只会让人不敢动，而把危险钉在一片地上，
//  「今晚上不上山砍柴」才成为一个真的选择 —— 那一带正好是林地、采石场、野果丛。
//  野物只在【夜里】和【冬天】下山：春夏白天还是那个安生的村子。
// ==========================================================================
const HILL_Y=11;                        // y < 11 是北山，往南是镇子和六户人家
const BEAST={
  野狗:{ n:'野狗', hp:3, spd:2.5, dmg:5,  bite:35, meat:1, hide:1,
         col:['#6b5a48','#40352b','#d8c8a8'] },
  山猪:{ n:'山猪', hp:6, spd:1.5, dmg:10, bite:70, meat:2, hide:1,
         col:['#4a3f38','#2c2521','#e0d6c4'] },
};
const BEASTS=[];
let _beastSeq=0, _beastHour=-1;
// 白天的春夏一只都没有；冬天最凶，夜里能有四只
function beastCap(){
  const night = clock<5*60 || clock>=19*60;
  if(seasonOf(day)==='冬') return night ? 4 : 2;
  return night ? 2 : 0;
}
function beastSpawn(){
  if(BEASTS.length>=beastCap()) return;
  // 全项目不用 Math.random：出没也走确定性哈希，重放才对得上
  const seed=day*911 + (clock/60|0)*37 + _beastSeq*13;
  if(hash01(seed) > 0.45) return;                    // 不是每个时辰都来
  const k = hash01(seed+7) > 0.72 ? BEAST.山猪 : BEAST.野狗;   // 山猪少见
  const gx = 2 + Math.floor(hash01(seed+11)*(GW-6));
  const gy = 1 + Math.floor(hash01(seed+19)*(HILL_Y-3));
  if(!solid[gy] || solid[gy][gx]) return;
  BEASTS.push({ k, n:k.n, hp:k.hp, gx, gy, px:(gx+.5)*T, py:(gy+.5)*T,
                ppx:(gx+.5)*T, ppy:(gy+.5)*T, face:1, anim:0,
                cd:0, ttl:60*8, id:_beastSeq++,
                side: hash01(seed+23)>0.5 ? 1 : -1 });   // 绕障碍固定偏哪一边
  emit('beast.came',{who:'山里', why:k.n, tags:['conflict']});
}
// 追人不用寻路（就这么几只，没必要跑 BFS），但也不能一头撞死在树上。
// 做法：把想走的方向依次拧 0° / ±52° / ±103° 试一遍，第一个走得通的就走 ——
// 五个候选足够绕开一棵林地，而【偏向哪一边由 b.side 定死】，
// 所以它是绕过去，不是在树前左右横跳。
//
// 【踩过的坑】最早写的是"两个轴都走不动才算撞死了，撞死了就侧移一步"。
// 人和狗在同一列时 dx≈0，目标 x 就是当前 x，free(nx,b.py) 恒为真 ——
// "撞死了"永远不成立，侧移一次都不会触发。实测追 3 格追了 750 模拟分钟。
// 后来改成看"这一步实际挪了没有"，又变成挪一步被拉回来一步地来回蹭。
// 判定"卡住"很难写对；换成"换个方向试"就没有判定这回事了。
function beastGo(b, ux, uy, dm){
  const sp=b.k.spd*dm*T;
  const free=(x,y)=>{ const gx=(x/T)|0, gy=(y/T)|0;
    return gx>=1 && gy>=1 && gx<GW-1 && gy<HILL_Y && !solid[gy][gx]; };
  for(const a of [0, 0.9*b.side, -0.9*b.side, 1.8*b.side, -1.8*b.side]){
    const c=Math.cos(a), sn=Math.sin(a);
    const nx=b.px+(ux*c-uy*sn)*sp, ny=b.py+(ux*sn+uy*c)*sp;
    if(free(nx,ny)){ b.px=nx; b.py=ny; b.gx=(b.px/T)|0; b.gy=(b.py/T)|0; return true; }
  }
  return false;
}
function beastBite(b, s){
  const armor = hasTool('甲', s) ? 0.5 : 1;      // 皮甲只对玩家管用（工具一向如此）
  const d=Math.round(b.k.dmg*armor);
  s.vigor=Math.max(0, s.vigor-d);
  s.need.energy=Math.max(0, s.need.energy-14);
  if(s.act){ emit('act.interrupted',
    {who:s.name, obj:s.act.o.n, why:'让'+b.n+'冲散了', tags:['act']}); s.act=null; }
  emit('beast.bit',{who:s.name, why:b.n, val:d, tags:['conflict']});
  s.flee=40;                                     // 挨了一口就往山下跑（见 step 里的逃命分支）
  fx(s.rx||s.px, (s.ry||s.py)-CH, '-'+d+' 体质', '#e0705a');
  chip(s.px, s.py-8, T*0.8, '#b03a2a');
  if(s.vigor<=0) die(s, b.n+'咬的');
}
function beastTick(dm){
  ...
  for(let i=BEASTS.length-1;i>=0;i--){
    ...
    for(const s of sims){
      ...
      const d=Math.hypot(s.px-b.px, s.py-b.py);
      if(d<td){ td=d; t=s; }
    }
    if(t && td<T*7){                             // 盯上了
      const dx=t.px-b.px, dy=t.py-b.py, d=Math.max(1,Math.hypot(dx,dy));
      if(td<T*0.9){ if(b.cd<=0){ b.cd=b.k.bite; beastBite(b,t); } }
      else beastGo(b, dx/d, dy/d, dm);
      b.face = dx>=0 ? 1 : -1;
      b.anim += dm*4.2;
    } else {                                     // 闲逛，逛够了回山里
...
//  只能靠镇上的公共去处凑合。茶棚一开始收钱，他就顶不住了 ——
//  冒烟测试里 starving 冒出来的那一个就是他。
//  两条改法里选了这条：与其把茶钱调回免费（那等于放弃「钱要有下水道」），
//  不如把「托管」这件事补完整 —— 托管的意思是"这人会自己过日子"，
//  而在这个版本里，会过日子的第一件事就是给自己立个窝。
//  只对玩家生效（NPC 那三样本来就自带），一天最多立一样，料不够就等明天。
// ==========================================================================
...
// ==========================================================================
const NPC_WISH=[
// 排序按【被动产出优先】。实测过：腌坛有两样山货时打分只有 11.3，而人一饿，
// 灶台是 50.4 —— 它永远排不上队，而山货又会被吃掉、被腌不掉、还会腐坏，
// 「手里正好攒着两样」的窗口极窄。所以腌坛排到最后：92 文买个基本用不上的坛子，
// 不如先把每天自己长东西的那几样置办齐。
...
// 料不够时只做几次比较就返回，那圈扫格子的开销只在真能造得起时才付。
function autoHomestead(){
  if(!autoPilot || !PC || PC.dead) return;
  for(const [key, art] of AUTO_HOME){
    const has = OBJ.some(o=>o.built && o.owner===PC.name &&
                            (art==='stove' ? !!o.stove : artOf(o)===art));
    if(has) continue;
    const d=BUILD[key];
    if(PC.inv.wood < d.wood || (PC.inv.stone|0) < (d.stone||0) || PC.money < d.money)
      continue;                       // 造不起就看下一样，别白扫一圈格子
    for(let r=2;r<=9;r++)
      for(let dy=-r;dy<=r;dy++) for(let dx=-r;dx<=r;dx++){
        const x=PC.gx+dx, y=PC.gy+dy;
        if(x<1||y<1||x>=GW-1||y>=GH-1) continue;
        if(!buildCheck(key,x,y)){ placeBuild(key,x,y); return; }
      }
  }
}

// 玩家动手：一下一下地打，和砍树是同一套手感（连按/长按 E）
function nearbyBeast(){
  let best=null, bd=T*1.7;
  for(const b of BEASTS){ const d=Math.hypot(b.px-PC.px, b.py-PC.py);
    if(d<bd){ bd=d; best=b; } }
  return best;
}
let _hitT=0;
function strike(){
  const b=nearbyBeast(); if(!b) return false;
  const now=performance.now();
  if(now-_hitT<SWING_CD) return true;            // 吃掉这次输入，但算「正在打」
  _hitT=now;
  b.hp -= TOOLS['叉'] ? 2 : 1;                   // 空手也能打，猎叉一下抵两下
  PC.swingT=1; PC.face = b.px>=PC.px ? 1 : -1;
  PC.need.energy=Math.max(0, PC.need.energy-2);
  chip((PC.px+b.px)/2, (PC.py+b.py)/2, T*0.8, '#b03a2a');
  fx(b.px, b.py-14, b.hp>0 ? '啪' : '倒了', '#e0b070');
  if(b.hp<=0){
    BEASTS.splice(BEASTS.indexOf(b),1);
    PC.inv.meat+=b.k.meat; PC.inv.hide+=b.k.hide;
    PC.skill+=0.35;                              // 打猎也是手艺
    emit('beast.slain',{who:PC.name, why:b.n, tags:['conflict']});
  }
  return true;
}
...
function step(dm){
  ...
  if(!autoPilot){
    ...
    if(s.act && s.act.phase==='do'){
      if(mv.l>0.1){
        ...
        s.moving=false;      // 力气活：进度只由「抡下去」推动，站着不动它不会自己好
      }
      else {
        ...
        const AD=advOf(s);
        for(const k in AD) if(k!=='provision')
          s.need[k]=Math.min(100,s.need[k]+AD[k]*sc*r/dur);
        for(const k in (o.cost||{}))
        ...
        if(o.social) for(const x of sims)
          if(x!==s && x.act && x.act.o===o && x.act.phase==='do'){
            ...
            if(s._co[x.name]>=CO_MEM){ s._co[x.name]=0;
              remember(s, x.name, 'social.攀谈', '一起待在'+o.n); }
            s.need.social=Math.min(100,s.need.social+o.adv.social*r/dur*0.5*mb(s,'socGain'));
          }
        s.act.left-=r;
        if(s.act.left<=0) finishAct(s);
      }
    }
    if(!s.act && mv.l>0.05){
      ...
      s.anim+=dm*WALK*3.5; s.moving=true;             // 迈得快了，脚步动画跟着走
      QF._d=(QF._d||0)+spd/T; if(QF._d>3) QF.moved=1;  // 引导第一步：走够三格算学会了
  ...
  for(const s of sims){
    ...
    // ---- 逃命 ----
    // 挨过咬之后，效用 AI 只会「重新打个分」：山上的活因为怕已经不值钱了，
    // 但他还站在原地，等他慢慢选完、慢慢走，第二口第三口已经下来了。
    // 实测冬天一周 33 次咬人，有人体质从 73 掉到 28。
    // 所以逃命要绕过决策：往南跑，跑出北山为止 —— 打不过，至少别站着挨。
    if(s.flee>0){
      s.flee-=dm;
      if(s.gy<HILL_Y){
        s.act=null;
        const sp=WALK*1.2*dm*T;                       // 逃命比平时快一点
        const free=(x,y)=>{ const gx=(x/T)|0, gy=(y/T)|0;
          return gx>=0&&gy>=0&&gx<GW&&gy<GH&&!solid[gy][gx]; };
        if(free(s.px, s.py+sp))      s.py+=sp;        // 先往南
        else if(free(s.px+sp, s.py)) s.px+=sp;        // 南边堵了就贴着绕
        else if(free(s.px-sp, s.py)) s.px-=sp;
        s.gx=(s.px/T)|0; s.gy=(s.py/T)|0;
        s.moving=true; s.anim+=dm*WALK*4;
        continue;
      }
      s.flee=0;                                       // 下了山就不跑了
    }
    ...
    if(s.act.phase==='go'){
      ...
      const o=s.act.o, r=Math.min(dm, s.act.left);
      // 分母必须是这一次的实际工时（手艺、工具、工时倍率都缩过它），不是物件的名义 dur。
      // 拿名义 dur 当分母的话，干得越快回得越少 —— 一加速就把人活活饿死。
      const dur=s.act.variantDur||o.dur;
      const AD=advOf(s);
      for(const k in AD) if(k!=='provision')
        s.need[k]=Math.min(100, s.need[k]+AD[k]*r/dur);
      for(const k in (o.cost||{}))
        s.need[k]=Math.max(0, s.need[k]-o.cost[k]*r/dur);
  ...
  if(_relTick>=5){ _relTick=0; refreshRel(); questTick(); autoHomestead(); }  // 每 5 模拟分钟
  ...
  for(let i=chips.length-1;i>=0;i--){ const c=chips[i];
    c.x+=c.vx*dm; c.y+=c.vy*dm; c.vy+=dm*12; c.t+=dm*0.9;
...
const BUILD={
  ...
  // ---- 后加的五样。挑的标准同样是「补上一个原来没有的处境」，
  //      而不是把已有的东西再摆一遍：
  //      茅厕/水井 —— 内急掉得最快（0.17/分），实测茅厕是全镇被选中最多的物件
  //                   （1577 次决策里 254 次）。这两样把每天最多的两趟路砍掉。
  //      菜畦     —— 野菜只有春天有，四季常驻之后野菜团/菜饭才立得住
  //      鱼塘     —— 院里的鱼，喂烤鱼/鱼羹/菌鱼羹这三道不费米的菜
  //      书斋     —— 手艺是唯一的长期成长，以前只能去镇上书塾
  //      其中两样吃石料 —— 采石场原先只服务谷仓/凉亭/义仓，用途太窄。
  // ---- 开局无家，这四样是「从头起家」的头四件 ----
  // 榻放在最前、也最便宜：没有它你根本睡不了觉，第一天就得把它立起来。
  榻:{ n:'榻', wood:6, money:10, w:2, h:1, art:'bed',
    tip:'睡觉的地方。没有它你只能熬着 —— 开局第一件',
    make(x,y,id){
      // 名字算出来，别写死第二人称 —— 邻居也会给自己盖榻，那张不叫「你的」。
      // 用 getter 而不是在这儿定死：读档 replay 时花名册还没重建、玩家也可能改名，
      // 当场算才不会错。
      return {id, art:'bed', built:1, owner:_builder.name, zone:'home', priv:1,
              get n(){ return this.owner===PC.name ? '你的榻' : this.owner+'的榻'; },
              x, y, w:2, h:1, use:[[x,y+1]],
              col:[BED,'#4a3628',BED2], adv:{energy:78}, dur:240 }; } },
  ...
  灶台:{ n:'灶台', wood:6, stone:3, money:20, w:1, h:1, art:'stove',
    ...
    make(x,y,id){
      return {id, art:'stove', stove:1, built:1, owner:_builder.name, zone:'home',
              ...
              pre:s=>!!bestRecipe(s), eff:s=>cook(s, s.act&&s.act.recipe),
              note:'生火做饭（费柴×1）',
              why:s=>s.inv.food>=1?'配料不够，只够干嚼':'没米下锅' }; } },

  // 树苗：几天后长成一棵真的林地。它的价值不在产量（山上的树一样砍），
  // 而在【位置】—— 家门口的柴，冬夜的野物够不着。
  树苗:{ n:'树苗', wood:2, money:15, w:1, h:1, art:'sapling',
    tip:'栽下六天长成林地。家门口的柴，野物够不着',
    make(x,y,id){
      ...
      return {id, built:1, owner:_builder.name, zone:'work', x, y, w:1, h:1, use:[[x,y+1]],
              ...
              get adv(){ return up()?{provision:26}:{fun:2}; },
              get dur(){ return up()?90:20; },
              get cost(){ return up()?{energy:16,hygiene:12}:{}; },
              get pre(){ return up()?null:(()=>false); },
              get why(){ return '还要 '+left()+' 天才成材'; },
              get note(){ return up()? '伐木 +'+(TOOLS['斧']?7:5) : '还要 '+left()+' 天'; },
              get eff(){ return up()? (s=>{ s.inv.wood += hasTool('斧',s)?7:5; }) : null; } }; } },
  ...
  菜畦:{ n:'菜畦', wood:8, money:25, w:1, h:1, art:'vegbed',
    ...
    make(x,y,id){ const sp={cap:1, left:1, built:1}; DAILY.push(sp);
      return {id, art:'vegbed', built:1, owner:_builder.name, zone:'home', daily:sp,
              ...
              get note(){ return sp.left>0 ? '采野菜 +1' : '今日已采尽'; },
              eff:s=>{ s.inv.veg++; sp.left--; return {type:'veg.pick'}; } }; } },
...
// ==========================================================================
//  合成表：全游戏的配方在这里【列一遍】
//  以前配方散在四处 —— RECIPE 的九道菜、TOOL 的五件家什、BUILD 的十种兴造，
//  还有硬写在 menuList() if-else 里的木器和腌坛。加一条要改三处，
//  界面上也永远凑不出一张「我现在能做什么」的总表 —— 这才是它不像个系统的根因。
//  数据仍然各归各处（菜在 RECIPE、工具在 TOOL、兴造在 BUILD），
//  CRAFT 只做那一份【索引】：菜单、合成面板、缺料提示全从它取，不再各写一份。
//  st = 在哪做。兴造没有工作台，站哪都能起手，所以单列 yard / town。
// ==========================================================================
const STATION={
  stove:{n:'灶台',       hint:'自家灶台'},
  bench:{n:'木作坊',     hint:'镇上两处，或自建工棚'},
  crock:{n:'腌坛',       hint:'自家院里建一个'},
  yard :{n:'自家院',     hint:'就地起手'},
  town :{n:'镇中心广场', hint:'就地起手'},
};
const ST_ORDER=['stove','bench','crock','yard','town'];
// 手艺分级：饥荒的科学机器在这里换成【手艺】。
// 37 条配方第一天全开等于没有长期目标；而手艺（生手→宗师七级）此前只管
// 工时/售价/精品，也没有目标感。把两者接起来 —— 手艺涨一级，表上多亮几行。
// 没列在这儿的默认 0 级：活得下去的那一套，生手就会。
const CRAFT_LV={
  锄:1, 鱼篓:1, 背篓:1, 铁斧:2, 铁镐:2, 猎叉:1, 皮甲:2,
  烤肉:0, 肉羹:1,
  菌鱼羹:1, 腌菜饭:2, 八宝饭:3,
  腌坛:1, 菜畦:1,
  // 起家和教程要用到的一律不设门槛。
  // 【踩过的坑】手艺分级是先于教程写的，两边一撞：教程第 12 步「开一块田」
  // 要新田（手艺1），第 14 步要树苗（手艺1），而前 11 步一点手艺也挣不到 ——
  // 砍树采石都不涨手艺，只有钓鱼/木作坊/书塾涨。新手会卡死在 12/15。
  榻:0, 米缸:0, 灶台:0, 新田:0, 茅厕:0, 树苗:0,
  药圃:2, 工棚:2, 水井:2,
  鸡棚:3, 书斋:3, 鱼塘:3, 水渠:3,
  谷仓:4, 水车:4, 凉亭:4, 义仓:5,
};
const CRAFT=[];
// 菜：灶台上的九道。柴写进 need —— 生火要柴，那也是一味「料」
for(const r of RECIPE){ r.lv=CRAFT_LV[r.n]||0;
  CRAFT.push({ id:'dish:'+r.n, n:r.n, st:'stove', lv:r.lv,
    need:Object.assign({wood:1}, r.need), out:'一顿饭',
    tip:'饱腹 +'+r.adv.hunger, recipe:r }); }
// 木器：整个经济里最值钱的一件
CRAFT.push({ id:'goods', n:'木器', st:'bench', need:{wood:2}, out:'木器×1',
  tip:'集市上最值钱的一件' });
// 家什：一次性的永久升级
for(const k in TOOL){ TOOL[k].lv=CRAFT_LV[TOOL[k].n]||0;
  CRAFT.push({ id:'tool:'+k, n:TOOL[k].n, st:'bench', lv:TOOL[k].lv,
    need: TOOL[k].hide ? {wood:TOOL[k].wood, hide:TOOL[k].hide} : {wood:TOOL[k].wood},
    money:TOOL[k].money, out:'家什（永久）',
    tip:TOOL[k].tip, tool:k }); }
// 腌货：山货不挑种类，凑够两样就行
CRAFT.push({ id:'pickle', n:'腌货', st:'crock', need:{forage:2}, out:'腌货×1',
  tip:'卖得比生货贵得多' });
// 兴造
for(const k in BUILD){ const d=BUILD[k]; d.lv=CRAFT_LV[k]||0;
  CRAFT.push({ id:'build:'+k, n:d.n, st:d.town?'town':'yard', lv:d.lv,
    need:d.stone?{wood:d.wood, stone:d.stone}:{wood:d.wood},
    money:d.money, out:'落成一座', tip:d.tip, build:k }); }
CRAFT.sort((a,b)=>ST_ORDER.indexOf(a.st)-ST_ORDER.indexOf(b.st));

const BARN={ n:0 };                      // 谷仓：抬高存粮在家计里的封顶
const MYPOND={ n:0 };                    // 自建鱼塘：每口塘要一个自己的钓运种子（POND 是地图上镇东那片水，别撞）

// ==========================================================================
//  吃的放不住（饥荒那一课）
//  在此之前 inv 里的东西是永不腐坏的计数，于是全村存粮一路涨到 317 斗，
//  上一轮只能给米铺加一条「够了别再籴」的刹车 —— 那是治标。腐坏是治本，
//  而且顺手给三样东西补上了它们本来就该有的理由：
//    腌坛 —— 山货会坏，腌货不会。它从「卖钱的小玩意」变成「过冬的办法」
//    谷仓 —— 兼任饥荒的冰箱（腐坏减到三成）。八宝饭那种跨季的料，
//            没有谷仓就是攒不住 —— 大菜第一次有了前置条件
//    集市 —— 手里的鱼是烫手的：今天不卖，明天就少小半
//  米不坏（谷物本来就是为了存），腌货不坏（腌的意义就在这），木石不坏。
// ==========================================================================
const SPOIL={ meat:0.5, fish:0.45, veg:0.22, fruit:0.22, mush:0.22, herb:0.16, egg:0.20 };
function spoilTick(){
  for(const s of sims){
    const slow=(s===PC && BARN.n>0) ? 0.35 : 1;          // 谷仓 = 冰箱
    s._rot=s._rot||{};
    const lost={};
    for(const k in SPOIL){
      const n=s.inv[k]|0;
      if(n<=0){ s._rot[k]=0; continue; }
      // 攒小数：一条鱼一天坏不掉「0.45 条」，攒够一整份才真的少一份
      s._rot[k]=(s._rot[k]||0)+n*SPOIL[k]*slow;
      while(s._rot[k]>=1 && (s.inv[k]|0)>0){ s.inv[k]--; s._rot[k]-=1;
        lost[k]=(lost[k]||0)+1; }
    }
    const ks=Object.keys(lost); if(!ks.length) continue;
    ks.sort((a,b)=>lost[b]-lost[a]);
    emit('econ.spoiled',{who:s.name, why:INV_N[ks[0]],
      val:ks.reduce((a,k)=>a+lost[k],0), tags:['econ']});
  }
}

// ==========================================================================
//  差事：一步一步把玩法教完
//  开局无家之后，新玩家面对的是一片空草地 —— 他既不知道该干什么，
//  也不知道【该往哪走】。所以这条链不只是目标清单，它是教程：
//    · 前六步一步只教一件事（走路 / 站到发光格 / 抡 / 攒够 / 开合成 / 落成）
//    · 每一步带一个 at()，地图和小地图上给指路标，出了视野屏幕边上给箭头
//    · 判据全是「你真的做了那个动作」，不是「你点了确定」
//  十五步之后交给祠堂 —— 四时供奉本来就是长线目标。
// ==========================================================================
const QF={};                            // 一次性动作的标记，进存档
function questWatch(e){
  if(e.who!==PC.name) return;
  const t=e.type;
  if(t.indexOf('econ.sold')===0)               QF.sold=1;
  else if(t==='farm.sown')                     QF.sown=1;
  else if(t==='beast.slain')                   QF.slain=1;
  else if(t==='cook.made')                     QF.cooked=1;
  else if(t==='act.done'){
    if(e.obj==='灶台')                          QF.cooked=1;   // 家常饭不发 cook.made
    else if(String(e.obj).indexOf('榻')>=0)      QF.slept=1;
    else if(e.obj==='林地')                     QF.chopped=1;
  }
}
onEvent(questWatch);
const mineIs = pred => OBJ.some(o=>o.built && o.owner===PC.name && pred(o));
// 指路：离玩家最近的、满足条件的物件的站位格
function nearUse(pred){
  let best=null, bd=1e9;
  for(const o of OBJ){ if(!pred(o) || !o.use || !o.use.length) continue;
    const u=o.use[0], d=Math.abs(u[0]-PC.gx)+Math.abs(u[1]-PC.gy);
    if(d<bd){ bd=d; best=u; } }
  return best;
}
const mineUse = pred => { const o=OBJ.find(x=>x.built&&x.owner===PC.name&&pred(x));
                          return (o&&o.use&&o.use[0]) || null; };
let questHide=false;                          // H 键收起引导
const QUEST=[
  {n:'先走两步', key:'WASD / 方向键',
   tip:'按住方向键走走看。人物是连续位移，不是一格一格跳。',
   done:()=>!!QF.moved},

  {n:'走到北山的林地下', key:'脚下会发光',
   tip:'顺着指路标往北走。走到树旁边，脚下那一格会亮起来 —— 亮了就说明够得着。',
   at:()=>nearUse(o=>o.tree && !o.built),
   done:()=>{ if(QF.chopped) return true; const n=nearby(); return !!(n && n.o.tree); }},

  {n:'连按 E 把树砍倒', key:'E / 空格',
   tip:'力气活是一下一下抡的：连按或按住 E，头顶的格子满了就成，四下一棵。',
   at:()=>nearUse(o=>o.tree && !o.built),
   done:()=>!!QF.chopped},

  {n:'弄够六根柴', key:'一棵五根',
   tip:'再砍一棵就够了。柴是这游戏里最要紧的东西 —— 做饭、盖屋、打家什都要它。',
   at:()=>nearUse(o=>o.tree && !o.built),
   done:s=>(s.inv.wood|0)>=6},

  {n:'按 C 打开合成', key:'C（手机点「合」）',
   tip:'合成表里列着全部配方：能做的排在前头，做不了的写着缺什么、该去哪做。',
   done:()=>!!QF.craftOpen},

  {n:'在草地上支起一张榻', key:'木6·10文',
   tip:'合成表里选「榻」→ 方向键挪光标 → E 落成。只要是草地就能盖，别人家的院子除外。',
   done:()=>mineIs(o=>artOf(o)==='bed')},

  {n:'回榻上睡一觉', key:'E',
   tip:'走到自己的榻上按 E。有些物件有好几种用法，会先弹菜单让你挑。',
   at:()=>mineUse(o=>artOf(o)==='bed'),
   done:()=>!!QF.slept},

  {n:'立个米缸', key:'木4·10文',
   tip:'缸里有米就饿不死 —— 冷食不用生火，是断粮时的最后一条退路。',
   done:()=>mineIs(o=>artOf(o)==='jar')},

  {n:'去采石场凿三块石料', key:'和砍树一样抡',
   tip:'北山西边那片石地就是采石场。灶台要石料 —— 这是石料的第一个用处。',
   at:()=>nearUse(o=>o.quarry),
   done:s=>(s.inv.stone|0)>=3},

  {n:'立起灶台，生火做一顿饭', key:'木6·石3·20文',
   tip:'生火做饭饱腹 78，冷食只有 38。每顿烧一根柴 —— 柴从此天天要惦记。',
   at:()=>mineUse(o=>!!o.stove),
   done:()=>!!QF.cooked},

  {n:'去集市换第一笔钱', key:'先得有东西卖',
   tip:'集市收木器、兽皮、腌货、兽肉、蛋、鲜鱼。手边最快的是钓鱼 —— 镇东有钓点。',
   at:()=>nearUse(o=>o.id==='stall'),
   done:()=>!!QF.sold},

  {n:'开一块田，种下去', key:'新田 木6·25文',
   tip:'草地随处可垦。开垦 → 播种（耗米1）→ 浇水 → 收割，庄稼要等好几天。',
   done:()=>!!QF.sown},

  {n:'打一件家什', key:'木作坊',
   tip:'镇上的木作坊能打铁斧、锄、鱼篓…… 工具折的是【下数】：铁斧砍树从四下变三下。',
   at:()=>nearUse(o=>o.craft),
   done:()=>Object.keys(TOOLS).length>0},

  {n:'在门口种棵树', key:'树苗 木2·15文',
   tip:'六天长成林地。产量和山上一样，值钱的是位置 —— 家门口的柴，冬夜的野物够不着。',
   done:()=>mineIs(o=>{const a=artOf(o); return a==='sapling'||a==='tree';})},

  {n:'打死一只野物', key:'入夜再去北山',
   tip:'天黑北山会下来野狗。打它和砍树是同一套手感：连按 E。倒了留下兽肉和兽皮。',
   at:()=>BEASTS.length?[BEASTS[0].gx,BEASTS[0].gy]:null,
   done:()=>!!QF.slain},
];
// 【差事只进不退】：好几条的判据是"手里有多少料"，而下一步往往正好把那些料花掉 ——
// 造灶台要三块石料，正是第 9 条刚让你凑的。不记最远进度的话，
// 落成的那一刻进度会退回上一条，读起来像做废了。
let _qMax=0;
function questAt(){
  let i=_qMax;
  while(i<QUEST.length && QUEST[i].done(PC)) i++;
  if(i>_qMax) _qMax=i;
  return _qMax;
}
function questCur(){ const i=questAt(); return i<QUEST.length?QUEST[i]:null; }
// 当前这条差事该去哪。拿不到就 null（比如"支起一张榻"哪儿都行）
function questSpot(){
  if(questHide) return null;
  const q=questCur();
  if(!q || !q.at) return null;
  try{ return q.at(); }catch(e){ return null; }
}
let _qAt=-1;
function questTick(){
  const cur=questAt();
  if(_qAt<0){ _qAt=cur; return; }              // 读档/开局：只对齐，不补发
  while(_qAt<cur){ emit('quest.done',{who:PC.name, why:QUEST[_qAt].n, tags:['quest']}); _qAt++; }
  _qAt=cur;
}


// 同一份木料，灶下叫柴、案上叫木料 —— 名字跟着用途走
const matName=(c,k)=> k==='forage' ? '山货'
                    : k==='wood'   ? ((c.st==='bench'||c.build)?'木料':'柴')
                    : (INV_N[k]||k);
// 缺什么。【这是「能不能做」唯一的判据】——菜单、合成面板、提示文案都问它，
// 三处各写一份判断迟早会对不上（T26/T27 的菜谱和供奉就是这么丢的）。
function craftLack(s, c){
  const miss=[];
  if(c.lv && skillLv(s)<c.lv) miss.push({k:'lv', n:'手艺 '+skillName(c.lv)});
  for(const k in c.need){
    const have = k==='forage' ? forageHave(s) : (s.inv[k]|0);
    if(have < c.need[k]) miss.push({k, n:matName(c,k), short:c.need[k]-have});
  }
  if(c.money && s.money < c.money)
    miss.push({k:'money', n:'文', short:Math.ceil(c.money-s.money)});
  return miss;
}
const craftGone = c => !!(c.tool && TOOLS[c.tool]);      // 打过的家什不再列
function craftNeedText(c){
  const t=Object.keys(c.need).map(k=>matName(c,k)+c.need[k]);
  if(c.money) t.push(c.money+'文');
  return t.join('·');
}
const missText = miss => '缺 '+miss.map(m=>
  m.k==='lv' ? m.n : m.k==='money' ? m.short+' 文' : m.n+m.short).join('、');
// 脚下是不是这条配方要的工作台（兴造不需要）
function stationNear(st){
  const n=nearby();
  if(!n || !n.ok) return null;
  const o=n.o;
  if(st==='stove' && o.stove)        return n;
  if(st==='bench' && o.craft)        return n;
  if(st==='crock' && o.art==='crock') return n;
  return null;
}
// 从合成面板直接开做：走的还是物件那条老路（attachSwing / finishAct），
// 只是省掉「先走过去、再按 E、再在菜单里找」这三步
function startCraftAt(n, c){
  const o=n.o, u=n.u;
  PC.px=(u[0]+.5)*T; PC.py=(u[1]+.5)*T; PC.gx=u[0]; PC.gy=u[1];
  PC.ppx=PC.px; PC.ppy=PC.py;
  const vdur=playerDur(o, c.tool ? TOOL[c.tool].dur : o.dur);
  PC.act=attachSwing({o, seat:u.join(), st:u, phase:'do', left:vdur, variantDur:vdur,
    scale:1, recipe:c.recipe||null, tool:c.tool||null});
  emit('act.start',{who:PC.name, obj:o.n+'·'+c.n, tags:['act']});
  if(PC.act.sw) swing();                     // 力气活：起手就是第一下
}
// 合成面板：能做的排在前头，做不了的留在后面（灰着，但看得见 ——
// 「看不见」正是这套东西以前最不像系统的地方）
function openCraftMenu(){
  QF.craftOpen=1;                                    // 引导：开过一次合成表
  const rows=CRAFT.filter(c=>!craftGone(c)).map((c,i)=>{
    const miss=craftLack(PC,c);
    const at  = c.build ? true : !!stationNear(c.st);
    return { a:c, i, ok: miss.length===0 && at,
             label: c.n+'（'+craftNeedText(c)+'）',
             sub: miss.length ? missText(miss) : (at ? '可做' : '去'+STATION[c.st].n) };
  });
  rows.sort((a,b)=>(a.ok?0:1)-(b.ok?0:1) || a.i-b.i);
  menu={kind:'craft', target:{n:'合成'}, idx:0, top:0, items:rows};
}
// 义仓每天开粥棚：喂镇上最饿的那个
...
const PCHOME=HOME[0];
// 能不能在这儿盖：【是草地就行】，不再问"是不是你家院里"。
// 只有一条禁令 —— 别人家的院子不许动。
const isGrass = (a,b)=> b>=0&&b<GH&&a>=0&&a<GW && FLOOR[b][a]===floorGrass;
const inNeighborYard = (a,b)=> HOME.some((h,i)=>
  i!==PLOT0 && a>=h.x && a<h.x+HW && b>=h.y && b<h.y+HH);
const canBuildAt = (a,b)=> isGrass(a,b) && !inNeighborYard(a,b);
...
function openBuildMenu(){
  ...
  menu={kind:'build', target:{n:'兴造'}, idx:0, items:keys.map(k=>{
    ...
    return { a:{key:k}, ok: PC.inv.wood>=d.wood && PC.inv.stone>=(d.stone||0)
                            && PC.money>=d.money && (!d.lv || skillLv(PC)>=d.lv),
...
const inHouse=(x,y)=>HOME.some((h,i)=>i!==PLOT0 && x>=h.x&&x<=h.x+6&&y>=h.y&&y<=h.y+5);
...
const FLOOR=Array.from({length:GH},(_,y)=>Array.from({length:GW},(_,x)=>{
  ...
  if(y>=25&&y<=36) return floorPath;             // 镇中心一带：夯土地面
  // 剩下的都是草地。以前这里兜底是夯土，草地只长在六户院子里 ——
  // 于是「只能建在自家院里」这条规矩其实是地形定死的。
  // 兜底改草之后，宅子之间的空档、镇子边上的边角，都成了可以开垦落脚的地方。
  return floorGrass;
}));
...
// 小地图是【按视口比例】给尺寸的，不能写死。
// MINI_S=2 在桌面的 432 宽上占 29%，搬到手机 7 格宽（168 世界像素）就占到 74% ——
// 盖住大半个上屏，还正好压在触屏的「✕」键上。
function miniScale(){
  return Math.max(1, Math.min(MINI_S,
    Math.floor(Math.min(VPW*0.36/GW, VPH*0.38/GH))));
}
function drawMini(cx,cy){
  const ms=miniScale();
  const w=GW*ms, h=GH*ms, ox=VPW-w-6, oy=6;
  ctx.globalAlpha=.82; ctx.fillStyle='#12100e'; ctx.fillRect(ox-2,oy-2,w+4,h+4);
  ctx.globalAlpha=1;
  ctx.drawImage(miniBg, ox, oy, w, h);           // 按比例缩，底图只烘一次
  ctx.strokeStyle='#c9a86a'; ctx.lineWidth=1;
  ctx.strokeRect(ox+cx/T*ms+.5, oy+cy/T*ms+.5,
                 VPW/T*ms-1, VPH/T*ms-1);
  for(const s of sims){
    ctx.fillStyle = s.isPlayer ? '#ffd155' : '#e0917f';
    const px=ox+(s.rx==null?s.px:s.rx)/T*ms, py=oy+(s.ry==null?s.py:s.ry)/T*ms;
    ctx.fillRect(px-1.5, py-1.5, 3, 3);
  }
  const sp=questSpot();                              // 差事指路：空心方块，和人的实心点分得开
  if(sp){ ctx.strokeStyle='#ffd155'; ctx.lineWidth=1;
    ctx.strokeRect(ox+sp[0]*ms-2.5, oy+sp[1]*ms-2.5, 5, 5); }
...
const FURN={
 ...
 // 树苗：一根细杆两片叶。成材之后换 tree 那张画，玩家一眼看得出好了没有
 sapling(x,y,w,h){ r(x+11,y+11,2,9,'#6b5334');
   r(x+6,y+9,6,3,'#4d6e3d'); r(x+7,y+8,4,2,'#5f8a4e');
   r(x+12,y+6,6,3,'#4d6e3d'); r(x+13,y+5,4,2,'#5f8a4e');
   r(x+8,y+19,8,2,'#5a4a34'); },
 // 菜畦：和药圃同一畦土，苗换成菜（采空了就只剩土）
 vegbed(x,y,w,h,o){ const dry=o&&o.daily&&o.daily.left<=0;
   r(x+1,y+8,w-2,h-10,'#6b5334'); r(x+2,y+9,w-4,h-12,'#8a7048');
   for(let i=0;i<3;i++) r(x+3,y+11+i*4,w-6,1,'#6b5334');
   if(!dry) for(const [a,b2] of [[4,6],[10,8],[16,6]]){
     r(x+a,y+b2+4,2,5,'#4d6e3d'); r(x+a-2,y+b2,6,5,'#6f9a4e');
     r(x+a-1,y+b2+1,4,2,'#8fbb62'); } },
 // 鸡棚：木栏 + 斜顶 + 两只鸡（拾完了就空着）
...
// 野物：低矮的四条腿 + 一只红眼睛。挨过打的头顶带血条。
function drawBeast(b){
  const c=b.k.col, bob=(Math.sin(b.anim)|0);
  ctx.globalAlpha=.25; ctx.fillStyle='#000';
  ctx.beginPath(); ctx.ellipse(b.rx,b.ry+2,8,3,0,0,7); ctx.fill(); ctx.globalAlpha=1;
  ctx.save();
  if(b.face<0){ ctx.translate(b.rx*2,0); ctx.scale(-1,1); }
  const X=Math.round(b.rx-9), Y=Math.round(b.ry-16)+bob;
  r(X-2,Y+5,4,3,c[0]);                                   // 尾
  r(X+1,Y+6,15,7,c[0]); r(X+2,Y+7,13,4,c[1]);            // 身
  r(X+2,Y+12,3,5,c[1]); r(X+11,Y+12,3,5,c[1]);           // 腿
  r(X+12,Y+2,7,6,c[0]); r(X+13,Y+3,5,4,c[1]);            // 头
  r(X+17,Y+5,4,2,c[2]);                                  // 嘴
  r(X+14,Y+3,2,2,'#e05a3a');                             // 眼
  ctx.restore();
  if(b.hp<b.k.hp) bar(b.rx-9, Math.round(b.ry-20), 18, b.hp/b.k.hp, '#c0503a');
}
...
// 力气活的进度：一下一格。连续的条会让人以为「等着就行」，格子才读得出「还差几下」。
function pips(x,y,w,done,need){
  ctx.fillStyle='#1a1614'; ctx.fillRect(x-1,y-1,w+2,5);
  const gw=(w-(need-1))/need;
  for(let i=0;i<need;i++){
    ctx.fillStyle = i<done ? '#ffd155' : '#3a332b';
    ctx.fillRect(x+i*(gw+1), y, Math.max(1,gw), 3);
  }
}
...
function drawMenu(cx,cy){
  ...
  const title = menu.kind==='npc'
    ...
    : menu.kind==='build'
      ...
      : menu.kind==='craft'
        ? '合成　柴/木 '+PC.inv.wood+' · 石 '+(PC.inv.stone|0)+' · '+Math.round(PC.money)+' 文'
        : menu.target.n;
  // 【窄屏要放得下】手机上视口只有 7 格 = 168 世界像素，菜单上限 152，
  // 而"八宝饭（柴1·米2·野果1·菌子1·蛋1） | 缺 野果1、菌子1、蛋1"要 286 ——
  // 原来只是把宽度夹到上限，文字照原样画，左边的名字和右边的说明会糊在一起。
  // 改成两道：先缩字号（9→7），还放不下就省略号，而且【先省说明再省名字】。
  ctx.textAlign='left';
  const cap=VPW-16;
  const need=fs=>{ ctx.font=fs+'px monospace';
    let m=ctx.measureText(title).width;
    items.forEach((it,i)=>{ m=Math.max(m, ctx.measureText(`${i+1}. ${it.label}`).width+14+
                                          ctx.measureText(it.sub).width); });
    return m+26; };
  let fs=9; while(fs>7 && need(fs)>cap) fs--;
  ctx.font=fs+'px monospace';
  let w=Math.min(cap, Math.max(150, need(fs))); menu._w=w; menu._fs=fs;
  const ell=(s,max)=>{ if(!s || ctx.measureText(s).width<=max) return s||'';
    while(s.length>1 && ctx.measureText(s+'…').width>max) s=s.slice(0,-1);
    return s+'…'; };
  const rowH=isTouch?22:17;
  // 滚动窗口：合成表有二十多条，一次全画会顶穿视口。
  // 只画光标周围的一屏，上下留箭头 —— 所有菜单共用这条逻辑。
  const maxRows=Math.max(4, Math.floor((VPH-56)/rowH));
  if(menu.top==null) menu.top=0;
  if(menu.idx<menu.top) menu.top=menu.idx;
  if(menu.idx>=menu.top+maxRows) menu.top=menu.idx-maxRows+1;
  menu.top=Math.max(0, Math.min(menu.top, items.length-maxRows));
  const view=items.slice(menu.top, menu.top+maxRows);
  const h=22+view.length*rowH+14;
  // 屏幕坐标：夹在当前视口里，而不是整张地图里
  // 触屏时右下角那列按钮是 DOM 浮层，菜单得给它让出 60css ÷ PXS 的世界像素
  const rightPad = isTouch ? 6+Math.ceil(64/Math.max(1,PXS)) : 6;
  const x=Math.round(Math.max(6,Math.min(VPW-w-rightPad, PC.rx-cx-w/2)));
  ...
  view.forEach((it,vi)=>{
    const i=menu.top+vi, ry=y+22+vi*rowH;
    if(i===menu.idx){ ctx.fillStyle='rgba(201,168,106,.22)'; ctx.fillRect(x+4,ry-1,w-8,rowH-2); }
    // 名字先占位，剩下多少才给说明；两边都放不下时名字也截 —— 但名字最后才截
    const lbl=ell(`${i+1}. ${it.label}`, w-24);
    const rest=w-24-ctx.measureText(lbl).width-6;
    const sub=rest>12 ? ell(it.sub, rest) : '';
    ctx.textAlign='left';
    ctx.fillStyle = !it.ok ? '#5f574c' : (i===menu.idx?'#ffd97a':'#d6cab0');
    ctx.fillText(lbl, x+9, ry+11);
    ctx.textAlign='right';
    ctx.fillStyle = !it.ok ? '#4a443c' : '#877c6b';
    ctx.fillText(sub, x+w-9, ry+11);
  });
  ctx.textAlign='left'; ctx.fillStyle='#6f6659';
  const more = items.length>view.length
    ? (menu.top>0?'▲':'　')+(menu.top+view.length<items.length?'▼':'　')
      +' '+(menu.idx+1)+'/'+items.length+'　' : '';
  ctx.fillText(more+'↑↓ 选择　Enter 确认　Esc 取消', x+8, y+h-5);
}
...
function render(){
  ...
  for(const b of BEASTS){
    b.rx = b.ppx==null ? b.px : b.ppx+(b.px-b.ppx)*alpha;
    b.ry = b.ppy==null ? b.py : b.ppy+(b.py-b.ppy)*alpha;
  }
  const ents=[...OBJ.map(o=>({y:(o.y+o.h)*T,o})), ...sims.map(s=>({y:s.ry==null?s.py:s.ry,s})),
              ...BEASTS.map(b=>({y:b.ry==null?b.py:b.ry, b}))]
    .sort((a,b)=>a.y-b.y);
  for(const e of ents){
    if(e.o){ drawObj(e.o); label(e.o); continue; }
    if(e.b){ drawBeast(e.b); continue; }
    ...
    // 抡下去的那一瞬：整个人朝目标前倾一下再落回来。精灵图没有专门的挥击帧，
    // 就借走路帧 + 一段位移 —— 这点位移是「打到了」唯一的体感来源，别省。
    const kk=s.swingT>0?Math.sin(s.swingT*Math.PI):0;
    const sox=Math.round(kk*4*(s.face||1)), soy=Math.round(-kk*3);
    const fr=s.swingT>0 ? (((1-s.swingT)*4)|0)%4
                        : (((s.act&&s.act.phase==='go')||s.moving)?(s.anim|0)%4:0);
    if(spriteOK){
      ctx.save();
      if(s.face<0){ctx.translate(dx+sox+CW,dy+soy);ctx.scale(-1,1);ctx.drawImage(cast,fr*SW,s.row*SH,SW,SH,0,0,CW,CH);}
      else ctx.drawImage(cast,fr*SW,s.row*SH,SW,SH,dx+sox,dy+soy,CW,CH);
      ctx.restore();
    } else {                                   // 降级：图挂了也能看 AI 跑
      ctx.fillStyle=['#8a6a42','#a8563c','#5f6a8c','#b09a4a'][s.row];
      ctx.fillRect(dx+sox+8,dy+soy+14,16,30);
      ctx.fillStyle='#e8dcc0'; ctx.fillRect(dx+sox+10,dy+soy+4,12,10);
    }
    ...
    if(s.act&&s.act.phase==='do'){
      if(s.act.sw) pips(s.rx-11,dy-6,22,s.act.sw.done,s.act.sw.need);
      else bar(s.rx-11,dy-6,22,1-s.act.left/(s.act.variantDur||s.act.o.dur),'#c9a86a');
    }
    // 最急需求的色点（一眼看出它在忙什么）
  ...
  // ---------- 指路标 ----------
  // 「不知道该往哪走」是新玩家最大的一道坎，光在侧栏写字是过不去的。
  const _spot=questSpot();
  if(_spot){
    const wx=(_spot[0]+.5)*T, wy=(_spot[1]+.5)*T;
    const k=.55+Math.sin(clock*0.9)*0.3;
    ctx.globalAlpha=k; ctx.strokeStyle='#ffd155'; ctx.lineWidth=2;
    ctx.strokeRect(_spot[0]*T+2, _spot[1]*T+2, T-4, T-4);
    ctx.globalAlpha=1; ctx.fillStyle='#ffd155';
    const ty=wy-T-2+Math.sin(clock*0.9)*2;
    ctx.beginPath(); ctx.moveTo(wx-5,ty); ctx.lineTo(wx+5,ty); ctx.lineTo(wx,ty+7); ctx.fill();
  }
  // ---------- 碎屑 ----------
  for(const c of chips){
    ctx.globalAlpha=Math.max(0,1-c.t/5);
    ctx.fillStyle=c.col; ctx.fillRect(Math.round(c.x),Math.round(c.y),2,2);
    ctx.globalAlpha=1;
  }
  // ---------- 浮动反馈 ----------
  ...
  document.body.classList.toggle('menuing', !!(menu||bmode));
  drawMini(cx,cy);
  // 目标不在视野里：贴着屏幕边缘给一个箭头，别让人满图乱找
  if(_spot){
    const sx=(_spot[0]+.5)*T-cx, sy=(_spot[1]+.5)*T-cy;
    if(sx<8||sy<8||sx>VPW-8||sy>VPH-8){
      let ex=Math.max(14,Math.min(VPW-14,sx)), ey=Math.max(20,Math.min(VPH-14,sy));
      // 右上角是小地图，箭头别画上去 —— 会和地图上的点位糊成一团
      const mw=GW*miniScale(), mh=GH*miniScale();
      if(ex>VPW-mw-12 && ey<mh+14) ey=mh+18;
      const a=Math.atan2(sy-VPH/2, sx-VPW/2);
      ctx.save(); ctx.translate(ex,ey); ctx.rotate(a);
      ctx.fillStyle='#12100e'; ctx.beginPath();
      ctx.moveTo(9,0); ctx.lineTo(-7,-7); ctx.lineTo(-7,7); ctx.fill();
      ctx.fillStyle='#ffd155'; ctx.beginPath();
      ctx.moveTo(7,0); ctx.lineTo(-5,-5); ctx.lineTo(-5,5); ctx.fill();
      ctx.restore();
    }
  }
  // 引导条：当前这一步 + 该按什么键。和侧栏那条同步，H 一起收
  if(!questHide){
    const _q=questCur();
    if(_q){
      ctx.font='9px monospace'; ctx.textAlign='left';
      // 窄屏放不下"标题＋按键"，按键那截先舍 —— 标题才是"现在该干什么"
      let t='▸ '+_q.n+(_q.key?'　'+_q.key:'');
      if(ctx.measureText(t).width+16 > VPW-12) t='▸ '+_q.n;
      const w=Math.min(VPW-12, ctx.measureText(t).width+16);
      ctx.globalAlpha=.9; ctx.fillStyle='#12100e'; ctx.fillRect(6,4,w,15);
      ctx.globalAlpha=1; ctx.strokeStyle='#6a5a3c'; ctx.lineWidth=1;
      ctx.strokeRect(6.5,4.5,w-1,14);
      ctx.fillStyle='#e8c169'; ctx.fillText(t, 14, 15);
    }
  }
...
// ---- 窄屏把长段折叠起来 ----
// 手机上整页要滚两千多像素。差事、行囊、需求是每回合都要看的；
// 家赀排名 / 祠堂 / 操作说明 / AI 打分 / 生产链 / 关系 是偶尔翻一次的 ——
// 后者默认收起，点标题展开。
// 【状态不能存在 DOM 里】：面板每 0.2 秒整个重画一次，
// <details open> 会在你松手之后立刻自己合上。所以记在 PSEC 里。
const PSEC={};                                   // {段名: 是否展开}，默认全收
function foldPanel(){
  if(innerWidth>1000) return;                    // 桌面不折，横着有的是地方
  for(const h of P.querySelectorAll('h2[data-sec]')){
    const open=!!PSEC[h.dataset.sec];
    h.style.cursor='pointer';
    h.insertAdjacentHTML('afterbegin', open?'▾ ':'▸ ');
    if(open) continue;
    for(let n=h.nextElementSibling; n && n.tagName!=='H2'; ){
...
function panel(){
  ...
  // ---- 行囊 ----
  // 米和柴永远显示：那是「今晚吃得上、吃得好不好」的两个数，每顿饭都要查。
  // 其余的有才露面，免得一屏都是 0。家什也在这儿 —— 打了工具却没地方看，
  // 等于白打（toolNames() 写好之后从来没被调用过）。
  const bagRow=(label, keys)=>{
    const t=keys.filter(k=>(PC.inv[k]|0)>0).map(k=>INV_N[k]+' '+(PC.inv[k]|0)).join('　');
    return t ? `<div class="row"><span class="dim">${label}</span><span>${t}</span></div>` : '';
  };
  const rice=PC?PC.inv.food|0:0, fire=PC?PC.inv.wood|0:0;
  const hotOK = PC && !!bestRecipe(PC);
  const coldOK= PC && (rice>0 || (PC.inv.fruit|0)>0);
  // 开局无家之后，「有没有灶台」不再是理所当然的 —— 这句提示以前只看柴和配料，
  // 一个连灶台都没造的人也会被告知"够生火做饭"。
  const usable = o => PC && (!o.owner || o.owner===PC.name || o.owner===spouseOf(PC.name));
  const myStove = PC && OBJ.some(o=>o.stove && usable(o));
  const myJar   = PC && OBJ.some(o=>artOf(o)==='jar' && usable(o));
  const kitchenTip =
      !myStove && !myJar ? '还没有灶台，也没有米缸'
    : !myStove           ? (coldOK ? '还没灶台，只能取冷食' : '还没灶台，缸里也空着')
    : fire>0             ? (hotOK ? '够生火做饭' : '有柴，缺配料')
    : coldOK             ? '没柴，只能取冷食'
                         : '没柴也没吃的';
  // ---- 差事 ----
  const qi=questAt(), qc=QUEST.length;
  const q=qi<qc?QUEST[qi]:null;
  const quest = !PC ? '' : questHide ? `
  <h2>差事 <span class="dim" style="font-weight:400;font-size:10px">${qi}/${qc} · 已收起，按 H 展开</span></h2>`
  : (q ? `
  <h2>差事 <span class="dim" style="font-weight:400;font-size:10px">${qi}/${qc}</span></h2>
  <div class="row"><span style="color:#e8c169">▸ ${q.n}</span>${
    q.key?`<span class="dim" style="font-size:10px">${q.key}</span>`:''}</div>
  <div class="dim" style="font-size:10px;line-height:1.65;padding:1px 0 3px">${q.tip}${
    q.at&&questSpot()?'<br><span style="color:#8fd0e8">地图上有指路标（小地图上是空心方块）</span>':''}</div>${
    qi>0?`<div class="row"><span class="dim" style="font-size:10px">✓ ${QUEST[qi-1].n}</span></div>`:''}
  <div class="dim" style="font-size:10px">按 H ${questHide?'展开':'收起'}引导</div>`
  : `
  <h2>差事 <span class="dim" style="font-weight:400;font-size:10px">${qc}/${qc}</span></h2>
  <div class="row"><span style="color:#8fd08a">起家的十二件都做完了</span></div>
  <div class="dim" style="font-size:10px;line-height:1.65;padding:1px 0 3px">
    往后看祠堂那四份供奉 —— 一年四季各一份，凑齐了祠堂就修成了。</div>`);
  const bag = !PC ? '' : `
  <h2>行囊 <span style="float:right;font-weight:400;color:#e8c169">${PC.money|0} 文</span></h2>
  <div class="row"><span class="dim">灶下</span><span>
    <b style="color:${rice>0?'#d6cab0':'#d07a72'}">米 ${rice}</b>　
    <b style="color:${fire>0?'#d6cab0':'#d07a72'}">柴 ${fire}</b>
    <span class="dim" style="font-size:10px">　${kitchenTip}</span></span></div>
  ${bagRow('山货', ['veg','fruit','mush','herb'])}
  ${bagRow('可售', ['goods','hide','pickle','meat','egg','fish','stone'])}
  <div class="row"><span class="dim">家什</span><span${Object.keys(TOOLS).length?'':' class="dim"'}>${
    Object.keys(TOOLS).length ? toolNames().join('　') : '空手 · 去木作坊打一件'}</span></div>`;
  P.innerHTML=`
  <div id="ctl">
    ${[['⏸',0],['▶',1],['▶▶',6],['▶▶▶',30]].map(([l,v])=>
  ...
  ${quest}
  ${bag}
  ${BEASTS.length ? `<div class="row"><span class="dim">北山</span>
    <b style="color:#e0705a">野物 ${BEASTS.length} 只 · ${
      [...new Set(BEASTS.map(b=>b.n))].join('、')}</b></div>` : ''}
  <h2 data-sec="rank">家赀排名</h2>
  ${sims.slice().sort((a,b)=>(b.money+b.inv.goods*MKT.furniture.p)-(a.money+a.inv.goods*MKT.furniture.p))
    .map((s,i)=>`<div class="row"><span class="${s===sel?'':'dim'}">${i+1}. ${s.isPlayer?'★ ':''}${s.name}</span>
      <span>${s.money|0} 文<span class="dim" style="font-size:10px"> · 米${s.inv.food} 柴${s.inv.wood} 器${s.inv.goods}${
        ['veg','fruit','mush','herb'].map(k=>s.inv[k]?' '+
          (Object.values(FORAGE).find(f=>f.key===k).n)+s.inv[k]:'').join('')
      }</span></span></div>`).join('')}
  <h2 data-sec="shrine">祠堂 <span class="dim" style="font-weight:400;font-size:10px">${
  ...
  <div class="dim" style="font-size:10px;line-height:1.8">
    ${isTouch
      ? `<b style="color:#e8c169">左下摇杆</b> 移动 &nbsp;·&nbsp;
         ...
         <b style="color:#e8c169">合</b> 合成 &nbsp;·&nbsp;
         <b style="color:#e8c169">造</b> 兴造 &nbsp;·&nbsp;
         点小人可切换查看<br>
         走到发光的格子上会弹出交互提示，菜单可直接点选。移动会打断正在做的事。<br>
         <b style="color:#e0a060">开局你没有家</b>：先砍柴，再在草地上支起榻、灶台、米缸。
         只要是草地就能盖、能开垦、能种树 —— 别人家的院子除外。<br>
         <b style="color:#e8c169">砍树、采石、采集、田里的活、木作</b>要一下一下地抡：
         按住「互动」连抡，头顶格子满了就成。工具和手艺折的是【下数】。<br>
      ...
      : `<b style="color:#e8c169">WASD / 方向键</b> 移动 &nbsp;·&nbsp;
         ...
         <b style="color:#e8c169">C</b> 合成 &nbsp;·&nbsp;
         <b style="color:#e8c169">B</b> 兴造 &nbsp;·&nbsp;
         <b style="color:#e8c169">H</b> 收起引导<br>
         <b style="color:#e0a060">开局你没有家</b>：先上北山砍柴，回来在草地上支起
         <b style="color:#e8c169">榻</b>（木6·10文），再立<b style="color:#e8c169">灶台</b>和
         <b style="color:#e8c169">米缸</b>。<b style="color:#e8c169">只要是草地就能盖、能开垦、能种树</b>
         —— 别人家的院子除外。<br>
         走到发光的格子上会弹出交互提示。移动会打断正在做的事。<br>
         <b style="color:#e8c169">砍树、采石、采集、田里的活、木作</b>要一下一下地抡：
         连按或按住 E，头顶格子满了就成。工具和手艺折的是【下数】。<br>
  ...
  <div class="row"><span class="dim">当前</span><b style="color:#e8c169">${
    a? (a.phase==='go' ? '前往 '+a.o.n
      : a.sw ? a.o.n+'　'+LABOR[a.sw.k].verb+' '+a.sw.done+'/'+a.sw.need+'　按 E 再来一下'
      : a.o.n+(a.o.note?'（'+a.o.note+'）':''))
     : (isPC?(autoPilot?'托管中…':'自由行动'):'思考中…')}</b></div>
  ...
  <h2 data-sec="ai">${isPC?(autoPilot?'托管中 · AI 正在按此决策':'AI 建议 · 你可以无视'):'它的决策依据 · 效用打分'}</h2>
  ...
  <h2 data-sec="chain">生产链 · 前置条件自动串联</h2>
  ...
  foldPanel();                                   // 窄屏折叠，必须在 innerHTML 之后
}
P.addEventListener('click',e=>{
  // 名字里现在套着 <span>（厢屋标记），点在标记上 e.target 就不是那个 <b> 了 ——
  // 得往上找一层，不然点到名字的一半会没反应。
  const el=(e.target.closest&&e.target.closest('[data-i],[data-kid],[data-sp],[data-auto],[data-reset],[data-sec]'))
           || e.target;
  if(el.dataset.sec!==undefined){ PSEC[el.dataset.sec]=!PSEC[el.dataset.sec]; panel(); return; }
  ...
  if(el.dataset.reset!==undefined){
    if(confirm('重开一局？当前村子的记忆、田地和账目都会清空。')){ resetAndReload(); }
    return; }
...
cv.addEventListener('click',e=>{
  if(ending){ resetAndReload(); return; }
  ...
  if(menu){                                          // 菜单打开时点击即选中（屏幕坐标）
    ...
    const i=(menu.top||0)+Math.floor((my-(y+21))/rowH);
    if(i>=(menu.top||0) && i<menu.items.length && mx>x && mx<x+w){ menu.idx=i; runMenu(); }
    else closeMenu();
    return;
  }
...
// ==========================================================================
// 12→13：加了 qf/qmax、built 记了栽种日、inv 多了兽肉兽皮，
// 而且玩家那块地从「自带屋子」变成了「空地」—— 老档读进来是半拉子世界，直接作废更干净
const SAVE_KEY='sim-game-save', SAVE_VER=13;
let _leaving=false;   // 要走了，封笔；见 resetAndReload

function saveGame(){
  // 终局之后不再写档：否则 pagehide 会把"死后的村子"存下去，
  // 而那份档里没有 isPlayer，读回来玩家会静默变成花名册里的第一个人。
  // 要走的路上同理，而且更隐蔽，见 resetAndReload。
  // 捏人还没落地时也不能存：那时候世界里站着的是默认的六个人。手机上切一下
  // App 就会触发 visibilitychange，刷新会触发 pagehide —— 只要存下去，
  // 下次进来 loadGame() 就成功了，捏人页再也不出现，玩家静默拿到默认村子。
  // 用「那个面板还在不在」来判断，而不是再立一个标记 —— 不可能跟画面对不上。
  if(ending || _leaving || creating()) return false;
  try{
    ...
    const data={ v:SAVE_VER, day, clock, autoPilot, speed, running, selIdx:sims.indexOf(sel),
      ...
      // 【攒了一半的进度也得存】。这些都是按天累加的小数或计数器，
      // 一天只走一点点，攒够了才兑现一次。不存就等于「读档清零」——
      // 而且不报错、画面上也看不出来，只是那个功能悄悄不生效了。
      // 实测：手里一份草药要攒 7 天才坏（0.16/天），每天读一次档就永远不坏，
      // 「食物腐坏」整个功能等于没开。_builtAt 同理：读档后全村能在同一天
      // 一起动土，置办家当的 6 天间隔形同虚设。
      sims:sims.map(s=>({ name:s.name, gx:s.gx, gy:s.gy, px:s.px, py:s.py, face:s.face,
        ...
        _rot:s._rot||null,         // 腐坏攒到哪儿了
        _builtAt:s._builtAt||null, // 上次置办家当是哪天
        _made:s._made||null,       // 做过几件活（手艺出精品按这个数轮）
        _co:s._co||null,           // 跟各人的摩擦/合作攒到哪儿了
        _frDay:s._frDay||null })) };
...
function loadGame(){
  ...
  try{
    ...
    for(const k in QF) delete QF[k];
    Object.assign(QF, d.qf||{});   // 睡过没、做过饭没 —— 这些看不出来，只能存
    _qMax=Math.max(0, Math.min(QUEST.length, d.qmax|0));   // 最远进度也得存，理由同上
    _qAt=-1;                       // 读档后重新对齐，别把老进度当成"刚做到"补发一串
    ...
    for(const sd of d.sims){
      ...
      Object.assign(s, { gx:sd.gx, gy:sd.gy, px:sd.px, py:sd.py, face:sd.face,
        need:sd.need, skill:sd.skill, money:sd.money, inv:fixInv(sd.inv), rel:sd.rel, mem:sd.mem||{},
        _gift:sd._gift, _vent:sd._vent, _fr:sd._fr, _frT:sd._frT,
        _blockN:sd._blockN, _crit:sd._crit,
        // 老存档没有这几样，读回来是 undefined —— 各处都有 ||{} / ||0 兜着，
        // 所以不用抬 SAVE_VER 作废老档
        _rot:sd._rot||{}, _builtAt:sd._builtAt||0, _made:sd._made||0,
        _co:sd._co||{}, _frDay:sd._frDay||{} });
...
// 重开一局：擦档 + 重载。_leaving 必须【先】立起来 —— location.reload() 自己会触发
// pagehide，而那上面挂着 saveGame，不封笔的话刚擦掉的档在那一瞬间被原样写回去：
// 下一次加载 loadGame() 照样成功，于是点几次「重开一局」都是同一个村子，
// 捏人页根本不出现。封笔只封这一条离场路径 —— wipeSave 本身不该有这个副作用，
// 「清个场、重新存一遍」是回归测试天天在做的事。
function resetAndReload(){ _leaving=true; wipeSave(); location.reload(); }
...
function drawEnding(){
  ...
  // 手机上没有键盘。点一下本来就管用（cv 的 click 里有 ending 分支），只是提示写错了
  ctx.fillStyle='#6f6659';
  ctx.fillText(isTouch?'点一下重开一局':'按任意键重开一局', x+W/2, y+H-10);
}
function drawDaily(){
...
function loop(now){
  ...
  if(PC.swingT>0) PC.swingT=Math.max(0, PC.swingT-dt*6);   // 挥击回弹跟真实时间，不跟倍速
...
// 捏人面板还开着 —— 这期间世界是「草稿」，不许写档，见 saveGame
function creating(){ return !!document.getElementById('create'); }
